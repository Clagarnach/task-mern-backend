...
test('should get tasks for a user', async () => {
    ...
    // Mocking the find method to return tasks for theuser
    ...
    // Ensure the response has the expected tasks
...
test('should set a new task for a user', async () => {
        ...
        const task = 
            {_id: 'new-task-id', text: 'New Task', user: 'user-id'};
        ...
        // Ensure the response contains the new task
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith(task);
        });

        test('should return a 400 error for missing task test', async () => {
...
test('should return a 401 error if user is not authorised to update the task', async () => {
    const taskId = 'task-id-1';
    const userId = 'user-id-2'; // different from the owner
    const req = { params: { id: taskId }, user: { id: userId }, body: { text: 'Updated Task' }};

    // Mocking the task to be updated, owned by different user
